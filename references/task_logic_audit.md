...
- Randomization/counterbalancing:
  - Trial list is generated by a custom condition-generation function as a deterministic random-walk sequence.
  - `randomize_within_block=false` keeps temporal drift order intact.
  - Seeded per block for reproducibility (`BlockUnit` block seed).
...
1. `pre_choice_fixation`
   ...
   - Valid keys: none
   - Timeout behavior: auto-advance after `pre_choice_fixation_duration`
...
2. `bandit_choice`
   ...
   - Timeout behavior: no key -> impute via configured no-choice policy (`random|left|right`) using preplanned trial fallback side
...
3. `choice_confirmation`
   ...
   - Valid keys: none
   - Timeout behavior: auto-advance after `choice_confirmation_duration`
...
4. `outcome_feedback`
   ...
   - Valid keys: none
   - Timeout behavior: auto-advance after `outcome_feedback_duration`
   - Next state: `iti`
5. `iti`
...
- Missing-response policy:
  ...
  - Impute choice using `condition_generation.no_choice_policy` (default: `random`), emit `choice_imputed`.
- Correctness logic:
...
- Reward/penalty updates:
  ...
  - `RewardTracker.update(...)` increments cumulative reward.
- Running metrics:
...
- Block:
  ...
  - `block_end` = 11
- Trial:
  - `pre_choice_fixation` -> `pre_choice_fixation_onset` = 20
...
- `main.py` runtime flow style (simple single flow / helper-heavy / why):
  - simple mode-aware runtime path with explicit block/trial logging.
- `utils.py` used? (yes/no)
  - yes.
- If yes, exact purpose (adaptive controller / sequence generation / asset pool / other):
  - drifting probability condition generation and reward draw helpers.
